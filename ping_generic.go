...
func (p *Pinger) pingGeneric(
	...
	if err != nil && p.UseStrict {
		return nil, err
	}

	status, err := p.pingSRV(pingFn, srvHost, int(srvPort))
	if err == nil {
		// Successfully pinged SRV-record server
		return status, nil
	}

	if !errors.Is(err, ErrNoSRV) {
		// Non-SRV related errors
		return nil, err
	}

	// Ping host/port as usual
	return pingFn(host, port)
}

func (p *Pinger) pingSRV(pingFn pingFunction, host string, port int) (interface{}, error) {
	// If SRV lookup is successful, check if there are any records
	if host != "" {
		status, err := pingFn(host, port)
		if err == nil {
	...
	// Ping using host/port instead of SRV
	return nil, ErrNoSRV
}
